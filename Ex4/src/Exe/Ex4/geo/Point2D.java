...
public class Point2D{
	...
	public void scale(Point2D cen, double ratio) {
		...
		vector = new Point2D(vector.x()*(1-ratio),vector.y()*(1-ratio)); // changes the distance between cen and the point by ratio
		this.move(vector);
	}
	public void rotate(Point2D cen, double angleDegrees) {
		double ogAngle = Math.toDegrees(Math.atan2( y()-cen.y(),x()-cen.x()));
		angleDegrees += ogAngle; // adds the chosen angle to the current angle from cen
		double angleRadians = Math.toRadians(angleDegrees);
		double r = this.distance(cen); // calcs the current distance from cen
		
		this._x = cen.x()+r*Math.cos(angleRadians); // changes the coords to the new angle with the same radius
		this._y = cen.y()+r*Math.sin(angleRadians);
		//System.out.println("x="+_x+" y="+_y);
	}
	/*public void rotate(Point2D cen, double angleDegrees, int num) { 
